...
REALTIME_SIMULATOR="$SCRIPT_DIR/realtime_data_simulator.py"  # 新增实时模拟器
...
case "${1:-help}" in
    ...
    "realtime")  # 新增实时模式
        echo "=== 实时数据上传验证模式 ==="
        check_service || exit 1
        if [ ! -f "$REALTIME_SIMULATOR" ]; then
            echo "正在创建实时模拟器..."
            # 这里会由下一个编辑命令创建文件
        fi
        python3 "$REALTIME_SIMULATOR"
        ;;
    "custom")
        echo "=== 自定义参数模式 ==="
        check_service || exit 1
