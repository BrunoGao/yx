...
class RealtimeSimulator:
    ...
    def generate_device_info(self, user):
        ...
        if random.random() < self.fake_device_probability:
            fake_device_sn = self.generate_fake_device_sn()
            if fake_device_sn:
    ...
    def run(self):
        ...
        print(f"总共生成了 {len(self.used_fake_devices)} 个虚假设备号: {', '.join(self.used_fake_devices)}")
