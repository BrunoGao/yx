...
# 配置
API_URL = "http://localhost:5001/health/upload"  # 更新为正确的API路径
BASELINE_API = "http://localhost:5001/api/baseline/generate"
DB_CONFIG = {'host':'127.0.0.1','user':'root','password':'123456','database':'ljwx','port':3306}
...
class HealthDataSimulator:
    ...
    def get_users(self, limit=None):
        ...
        try:
            ...
            sql = "SELECT id,user_name,customer_id,org_id,org_name, device_sn FROM sys_user WHERE device_sn IS NOT NULL AND device_sn != '' AND device_sn != '-' AND is_deleted = 0"
    ...
    def generate_realistic_data(self, device_sn, timestamp, user_profile=None):
        ...
        # 100米范围内的随机偏移
        lat_offset = random.uniform(-0.0009, 0.0009)  # ±100米纬度偏移
        lng_offset = random.uniform(-0.0011, 0.0011)  # ±100米经度偏移
